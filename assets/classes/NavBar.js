...
class NavBar extends Estilo {
  ...
  abrirMoreButton() {
    ...
    if (this.moreButton.ref.style.display != 'flex') {
      ...
      let anim = this.moreButton.ref.animate([{ width: '0px', opacity: 0 }, { width: '80px', opacity: 1 }], 200)
      anim.onfinish = () => this.addEstilo(this.moreButton.text, { display: 'flex' })
    }
  ...
  fecharMoreButton() {
    ...
    if (this.tabs.length <= this.limiteAbas) {
      ...
      if (this.moreButton.ref.style.display != 'none') {
        let anim = this.moreButton.ref.animate([{ width: '80px', opacity: 1 }, { width: '0px', opacity: 0 }], 200)
  ...
  abrirMoreModal() {
    ...
    if (this.moreModalAberto) {
      let anim = this.moreNavTabs.animate([{ opacity: '1' }, { opacity: '0' }], 100)
      anim.onfinish = () => {
      ...
      let anim = this.moreNavTabs.animate([{ top: '4vh', opacity: '0' }, { top: '6vh', opacity: '1' }], 100)
  ...
  atualizarNumAbas() {
    this.moreButton.text.textContent = `${(this.tabs.length) - this.limiteAbas} mais`
  ...
  novaAba(tabsContainer, moreContainer) {
    ...
    if (this.tabs.length >= this.limiteAbas) {
      ...
      this.tabs.push(new NavTabs({ text: 'Folha 1', ref: moreContainer, tabs: this.tabs, moreTabs: true, navbar: this, index: (this.tabs.length + 1) }))
      console.log(this.tabs)
      this.atualizarNumAbas()
    } else {

      this.tabs.push(new NavTabs({ text: 'Folha 1', ref: tabsContainer, moreTabsRef: moreContainer, tabs: this.tabs, navbar: this, index: (this.tabs.length + 1) }))
      console.log(this.tabs)
    }
